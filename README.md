...
<h2>1. What is Distributed Logging?</h2>
<p>
Distributed Logging is the practice of collecting, storing, and analyzing logs generated by
multiple services (usually microservices) in a centralized and structured way.
</p>

<p>
In a typical Spring Boot application, logs are generated at runtime to record events such as:
</p>
<ul>
...
<p>
SLF4J is <strong>not a logging implementation</strong>. It is a <strong>facade (abstraction layer)</strong>
that provides a common API for various logging frameworks in the Java ecosystem.
</p>

<p>
Common SLF4J implementations include:
</p>
<ul>
  <li>Logback (default in Spring Boot)</li>
  <li>Log4j2</li>
  <li>java.util.logging</li>
</ul>

<p>
Because SLF4J follows the <strong>Facade Design Pattern</strong>, it exposes only APIs and hides
the actual logging implementation. This allows developers to switch logging frameworks
without changing application code.
</p>

---

<h2>3. Logging Implementation in Spring Boot</h2>

<p>
By default, Spring Boot ships with <strong>Logback</strong> as the SLF4J implementation.
</p>

<p>
If you want to use <strong>Log4j2</strong> instead, you must explicitly add the Log4j2 dependency
and exclude Logback.
</p>

<p>
Important:
Having multiple SLF4J implementations on the classpath is dangerous because SLF4J may pick
one <em>randomly</em>, which is unacceptable in production systems.
</p>

---

<h2>4. LoggerFactory and Logger Creation</h2>

<p>
SLF4J provides a <code>LoggerFactory</code> that is responsible for creating and managing logger instances.
</p>
...
<ul>
  <li><code>LoggerFactory</code> delegates to <code>ILoggerFactory</code></li>
  <li>The concrete implementation (Logback / Log4j2) decides which logger to return</li>
  <li>Each class gets exactly <strong>one logger instance</strong></li>
</ul>

<p>
Logger instances are cached using the class name as the key.
If a logger already exists for a class, it is reused.
</p>

---

<h2>5. Logging Levels</h2>

<p>
Logging levels determine which log statements are printed.
</p>
...
<p>
If a logger is set to <strong>INFO</strong> level:
</p>
<ul>
  <li>INFO, WARN, ERROR → Printed</li>
  <li>DEBUG, TRACE → Ignored</li>
</ul>
...
<p>
If no level is defined, the default level is <strong>INFO</strong>.
</p>

---

<h2>6. Logger Hierarchy</h2>

<p>
Logging follows a hierarchical structure:
</p>

<ul>
  <li>The <strong>root logger</strong> always exists</li>
  <li>All other loggers are children of the root logger</li>
</ul>

<p>
If a child logger accepts a log event, the event is propagated upward to all parent loggers.
</p>

<p>
Important behavior:
Once a log event is accepted by a child logger, <strong>all parent appenders execute</strong>,
regardless of the parent logger’s level.
</p>

---

<h2>7. What is an Appender?</h2>

<p>
An <strong>Appender</strong> is the component that decides <em>where</em> a log will be written.
</p>

<p>Common appenders include:</p>
<ul>
  <li>Console</li>
  <li>File</li>
  <li>Database</li>
  <li>Kafka</li>
  <li>Cloud logging systems</li>
</ul>

<p>
Example:
If a file appender is configured, logs are written to a file like:
</p>
...
<h2>8. Why Logs Appear on Console Without Configuration?</h2>

<p>
Even if no appender is explicitly configured, logs still appear on the console.
</p>
...
<ul>
  <li>The <strong>root logger</strong> always has a default <strong>Console Appender</strong></li>
  <li>If no appenders are defined at lower levels, logs fall back to the root logger</li>
</ul>

---

<h2>9. Log Aggregation Tools</h2>

<p>
Once logs are generated, they are usually consumed by <strong>log aggregation tools</strong>
for centralized analysis and monitoring.
</p>

<p>Common log aggregation tools include:</p>
<ul>
  <li>ELK Stack (Elasticsearch + Logstash + Kibana)</li>
  <li>Datadog</li>
  <li>Promtail</li>
  <li>Cloud-native logging platforms</li>
</ul>

<p>
These tools read logs from their output destinations (files, streams, or agents)
and make them searchable and visualizable.
</p>
...
<ul>
  <li>SLF4J is an abstraction, not a logging implementation</li>
  <li>Logback is the default logging provider in Spring Boot</li>
  <li>Only one SLF4J implementation should exist in production</li>
  <li>Loggers follow a hierarchical structure</li>
  <li>Root logger always has a console appender by default</li>
  <li>Log aggregation tools centralize logs across services</li>
</ul>

#<h2>APPENDERS</h2>

<img width="1417" height="732" alt="image" src="https://github.com/user-attachments/assets/29ffc853-a843-4288-856e-e928beecc61b" />
<img width="1138" height="457" alt="image" src="https://github.com/user-attachments/assets/d5c6aa13-cb34-48b4-95a5-b20aa7101cf9" />

<h2>What is an Appender?</h2>

<p>
An <strong>Appender</strong> is the component that decides <strong>where your logs will go</strong>.
You can think of it as a helper that delivers log messages to different destinations.
</p>

<p>
While a <strong>Logger</strong> decides <em>what</em> to log,  
an <strong>Appender</strong> decides <em>where</em> to send that log.
</p>

<p><strong>Common types of Appenders:</strong></p>

<ul>
  <li><strong>Console Appender</strong> – prints logs to the terminal</li>
  <li><strong>File Appender</strong> – writes logs to a file</li>
  <li><strong>Database Appender</strong> – stores logs in a database</li>
  <li><strong>Kafka / Messaging Appender</strong> – sends logs to a message broker</li>
</ul>

<hr/>

<h2>Appenders in Logback vs Log4j2</h2>

<p>
Both <strong>Logback</strong> and <strong>Log4j2</strong> use the concept of Appenders
to define log destinations.
</p>

<p>
The core idea remains the same, but they differ slightly in:
</p>

<ul>
  <li>Configuration syntax</li>
  <li>XML structure</li>
  <li>Advanced features and plugins</li>
</ul>

<p>
In this project, we are using <strong>Logback</strong>, which is the
<strong>default logging framework for Spring Boot</strong>.
</p>

<hr/>

<h2>Logback Configuration File in Spring Boot</h2>

<p>
To configure Logback, Spring Boot looks for a specific file by default:
</p>
...
<p>
The file name and location are important.  
Spring Boot automatically searches for <code>logback-spring.xml</code>,
So this format should be followed.
</p>

<hr/>

<h2>What Does <code>logback-spring.xml</code> Contain?</h2>

<p>
This configuration file defines the complete logging behavior of the application.
It usually contains:
</p>

<ul>
  <li>All <strong>Appenders</strong></li>
  <li>Log <strong>Patterns</strong> (log format)</li>
  <li>All custom <strong>Loggers</strong></li>
  <li>The <strong>Root Logger</strong></li>
</ul>

<p>
Using this file, we can precisely control:
</p>

<ul>
  <li>Log levels (INFO, DEBUG, ERROR, etc.)</li>
...
<h2>Log Level Priority</h2>

<p>
Log levels can be configured in two places:
</p>
...
<p>
<strong>Important:</strong><br/>
If the same logger level is defined in both files,
the configuration in <code>logback-spring.xml</code>
<strong>takes higher priority</strong>.
</p>

<hr/>
<img width="1683" height="299" alt="image" src="https://github.com/user-attachments/assets/2a405931-2d27-4804-8ee8-d5db02a3c727" />

<img width="1458" height="985" alt="image" src="https://github.com/user-attachments/assets/50315fe0-9817-4d47-b5d8-cb1b1ae7f4e3" />
<img width="1740" height="942" alt="image" src="https://github.com/user-attachments/assets/4af396f8-d18b-4b8c-ae15-c579379b656e" />
<img width="1398" height="169" alt="image" src="https://github.com/user-attachments/assets/1e5baedc-e8b4-4692-b2d3-ed447435e3e4" />

<h3>Additivity</h3>
<p>
<strong>Additivity</strong> defines whether a log event generated by a logger
is passed to the <strong>parent logger</strong> in the hierarchy.
</p>
<p>
In simple terms, it controls <strong>how far a log message travels</strong>
up the logger chain.
</p>
...
<ul>
  <li>When <strong>additivity = true</strong> (default):</li>
  <ul>
    <li>The log is written by the current logger</li>
    <li>The same log is also passed to parent loggers</li>
    <li>This can result in <strong>duplicate log entries</strong></li>
  </ul>
</ul>

<ul>
  <li>When <strong>additivity = false</strong>:</li>
  <ul>
    <li>The log is handled only by the current logger</li>
    <li>The log does <strong>not propagate</strong> to parent loggers</li>
    <li>This helps avoid duplicate logs</li>
  </ul>
</ul>

<p>
Additivity is commonly used when you want specific logs to go to a specific
appender (for example, writing errors to a separate file).
</p>
...
<ul>
  <li>Controls log propagation to parent loggers</li>
  <li>Default value is <strong>true</strong></li>
  <li>Useful for preventing duplicate logs</li>
  <li>Configured inside <code>logback-spring.xml</code></li>
</ul>

<h3>Example</h3>
<img width="1713" height="526" alt="image" src="https://github.com/user-attachments/assets/dbc24ee4-9e47-401a-91f9-70e61cbe5455" />
